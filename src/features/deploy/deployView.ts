    ...
    <div class="flex flex-col gap-4">
      ...
      <section class="bg-surface border border-line rounded-xl shadow-card p-5">
        ...
        <ol class="flex flex-col gap-2.5">
          ${step(1, `打开 <code class="${CODE_CLS}">dash.cloudflare.com</code> → Workers & Pages →「创建 Worker」（模板选 Hello World）→「编辑代码」。`)}
          ${step(2, `全选删除模板代码，点下方「复制源码」粘贴进去，然后点右上角「部署」。`)}
          ${step(3, `进入「设置 → 变量和机密」，添加下方 4 个环境变量（<code class="${CODE_CLS}">API_KEY</code> 类型选「机密」）。`)}
          ${step(4, `进入「设置 → 绑定」→「添加绑定」→ R2 存储桶，绑定名称填 <code class="${CODE_CLS}">IMAGES</code>（首次需先在 R2 页面创建存储桶）。`)}
          ${step(5, `回到本应用「设置」页，把 Worker 地址（<code class="${CODE_CLS}">https://xxx.workers.dev</code>）和 API Key 填入并保存。`)}
        </ol>
      </section>
      ...
      <section class="bg-surface border border-line rounded-xl shadow-card p-5">
        ...
        <p class="text-xs text-ink3 leading-relaxed mb-3.5">
          <code class="${CODE_CLS}">src/index.ts</code> — Storage Gateway（上传 / 列表 / 删除 / 统计），整体粘贴到 Worker 编辑器，不要改动。
        </p>
        ...
        <div class="overflow-hidden rounded-lg border border-line">
          <table class="w-full text-[12.5px]">
            ...
            <tbody class="text-ink2 divide-y divide-line">
              ...
              <tr><td class="px-3 py-2 font-mono text-ink">PUBLIC_BASE_URL</td><td class="px-3 py-2">是</td><td class="px-3 py-2">R2 自定义域名，如 https://img.example.com（结尾无斜杠）</td></tr>
